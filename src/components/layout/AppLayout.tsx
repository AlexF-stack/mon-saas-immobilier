...
import { MonitoringUserContext } from '@/components/MonitoringUserContext'
...
export interface AppLayoutProps {
  ...
  role?: string
  userProfile?: { id?: string; name: string | null; email: string }
}
  ...
  return (
    <div className="bg-grid noise-overlay flex min-h-screen overflow-x-clip bg-background text-primary">
      ...
      <div className="flex-1 flex min-h-screen min-w-0 flex-col">
        <MonitoringUserContext userId={userProfile?.id ?? null} email={userProfile?.email ?? null} />
        <AppHeader
