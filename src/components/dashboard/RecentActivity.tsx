  ...
  return (
    <ul className={cn('space-y-0', className)}>
      {items.map((item) => {
        ...
        const content = (
          <>
            ...
            <div className="shrink-0 text-right">
              {item.amount != null && (
                <p className="text-sm font-medium tabular-nums text-slate-900 dark:text-slate-100">{item.amount.toLocaleString('fr-FR')} FCFA</p>
              )}
        ...
        const wrapperClass =
          'flex items-center gap-3 rounded-xl px-3 py-2.5 transition-colors duration-150 hover:bg-slate-50 dark:hover:bg-slate-800/50'
        return (
          <li key={item.id}>
            {item.href ? (
