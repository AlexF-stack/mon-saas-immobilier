...
import { sendDailyPaymentReminders } from '@/lib/payment-reminders'
...
export async function GET(request: Request) {
  try {
    ...
    const rateLimitError = await enforceRateLimit({
      request,
      bucket: 'PAYMENT_REMINDER_CRON',
      limit: CRON_RATE_LIMIT,
      windowMs: CRON_RATE_WINDOW_MS,
      message: 'Too many payment reminder cron calls.',
      extraKey: 'daily',
    })
    if (rateLimitError) {
    ...
    logServerEvent({
      event: 'internal.payment_reminder.daily.requested',
      correlationId,
      route,
    })
    ...
    logServerEvent({
      event: 'internal.payment_reminder.daily.completed',
      correlationId,
      route,
      details: summary,
    })
    ...
    await captureServerError(error, {
      scope: 'internal_payment_reminder_daily',
      targetType: 'PAYMENT_REMINDER',
      correlationId,
      route,
      event: 'internal.payment_reminder.daily.failed',
    })
